...
async def download_and_send_media(bot, chat_id, url, media_type):
    # Параметри для завантаження найкращого формату без злиття
    ydl_opts = {
    ...
    try:
        start_time = time.time()  # Початок завантаження

        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=True)
            filename = ydl.prepare_filename(info)

        end_time = time.time()  # Кінець завантаження
        elapsed_time = end_time - start_time

        # Використання FSInputFile для відправки
